...
/*
 * 采用TensorRT的API结构构建模型时，需要申请资源和释放资源，很容易忘记释放，导致内存泄漏
 * 这一章节我们采用智能指针改进lesson_02中的resnet18分类器
 * 这里只是演示智能指针的使用方法，因此只对model_inference函数进行了改进，其它代码与lesson_02中一致
*/
...
// 配合智能指针用于释放TensorRT资源的函数
template <class _T>
void nvdestroy(_T* nvptr)
{
    if(nvptr)
    {
        printf("Destroy Nvidia %p.\n", nvptr);
        nvptr->destroy();
    }
}
...
void model_inference()
{
    ...
    // 在这里采用智能指针管理内存
    auto runtime = std::shared_ptr<nvinfer1::IRuntime>(nvinfer1::createInferRuntime(logger), nvdestroy<nvinfer1::IRuntime>);
    auto engine = std::shared_ptr<nvinfer1::ICudaEngine>(runtime->deserializeCudaEngine(model_data.data(), model_data.size()), nvdestroy<nvinfer1::ICudaEngine>);
    auto context = std::shared_ptr<nvinfer1::IExecutionContext>(engine->createExecutionContext(), nvdestroy<nvinfer1::IExecutionContext>);
